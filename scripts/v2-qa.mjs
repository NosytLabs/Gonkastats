...
try{
 ...
 await page.goto(base+'/agents',{waitUntil:'networkidle'});await page.getByRole('button',{name:'All definitions',exact:true}).click();await expect(page.getByText('Documentation only',{exact:true}).first()).toBeVisible();report.checks.push('Write definitions are visibly documentation-only');
