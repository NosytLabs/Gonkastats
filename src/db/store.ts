...
export async function history(hours=24):Promise<HistoryPoint[]>{if(!process.env.DATABASE_URL)return [];const rows=await db().select().from(snapshots).where(gte(snapshots.slot,new Date(Date.now()-Math.min(168,Math.max(1,hours))*3600000))).orderBy(desc(snapshots.slot)).limit(2048);return rows.reverse().map(({slot,data})=>({at:slot.toISOString(),weight:data.participants.length?sum(data.participants.map(p=>p.weight)):null,price:data.sources.find(s=>s.id==='pricing')?.status==='recent'?data.fx:null,participants:data.participants.length||null}));}
