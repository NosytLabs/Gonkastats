...
import {curlCommand} from '@/core/community-rules';
import {PageHeading,RawRecord} from './shared';
function ReferenceCard({definition:d,origin}:{definition:ApiDefinition;origin:string}){
 ...
 <pre className="code-sample">{curlCommand(origin,path)}</pre>
 <div className="inline-controls"><CopyButton text={curlCommand(origin,path)} label="Copy curl"/><button className="button primary" disabled={busy} onClick={run}><Play size={13}/>{busy?'Reading…':'Try it'}</button>{http!==null&&<Badge tone={http<400?'green':'amber'}>HTTP {http}</Badge>}</div>
 {result!==null&&<div className="api-result"><RawRecord data={result}/></div>}</div></details>;
}
export function ApiReference({s}:{s:Snapshot}){
 const [origin,setOrigin]=useState('http://localhost:3000');useEffect(()=>setOrigin(window.location.origin),[]);
