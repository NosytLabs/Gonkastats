...
{id:'participantStats',name:'Participant reputation stats',scope:'chain',coverage:'Source-reported participant reputation, reward counters and completed epochs. Not an independent performance rating.',docs:'https://rpc.gonka.gg/chain-api/productscience/inference/inference/participants_stats'},
{id:'dex',name:'Wrapped GNK on Uniswap',scope:'provider',coverage:'Public DEX quote for the wrapped representation, not the native chain supply. Not a settlement or a full market depth claim.',docs:'https://app.uniswap.org/explore/tokens/ethereum/0x972a7A92D92796a98801A8818bcF91f1648f2F68'},
...
export function emptySnapshot():Snapshot{return {version:1,generatedAt:new Date().toISOString(),mode:'live',sources:[],epoch:null,participants:[],participantStats:[],dex:null,models:[],networkModels:[],governanceModels:[],pocModels:[],devshardVersions:[],versions:{},endpointDeclaredTotal:null,blocks:[],proposals:[],hardware:[],stats:[],fx:null,fxAt:null,totalSupply:null,tokenomics:{},communityPool:null,protocol:{},validators:null,statsWindow:null,endpoints:[],catalogAuth:null};}
export function normalizeSnapshotShape(s:Snapshot):Snapshot{return {...s,participantStats:s.participantStats??[],dex:s.dex??null,networkModels:s.networkModels??[],governanceModels:s.governanceModels??[],pocModels:s.pocModels??[],devshardVersions:s.devshardVersions??[],versions:s.versions??{},endpointDeclaredTotal:s.endpointDeclaredTotal??null,endpoints:s.endpoints??[]};}
